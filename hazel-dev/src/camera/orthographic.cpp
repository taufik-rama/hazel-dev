...
void Orthographic::set_projection(float left, float right, float bottom,
                                  float top) {
...
void Orthographic::recalculate_matrix() {
  ...
  if (this->translation_rotation ==
      TRANSLATION_ROTATION::ROTATE_BEFORE_TRANSLATE) {
    transformation = translation * rotation;
  } else {
    transformation = rotation * translation;
  }

  // `transformation` is the object transformation, `view_matrix` is the
  // camera's. When the camera moves left, the object moves right, so it's
  // always inverted.
