...
static GLenum
shader_data_type_to_opengl_type(hazel::renderer::ShaderDataType t) {
  switch (t) {
...
void VertexArray::add_vertex_buffer(
  ...
  HAZEL_DEV_LOG_TRACE("Adding vertex buffer (id: {}) to vertex array (id: {})",
                      *buffer, this->renderer_id);
  ...
  for (const auto &e : layout) {
    ...
    gl_call(glVertexAttribPointer(index, e.get_component_count(),
                                  shader_data_type_to_opengl_type(e.type),
                                  e.normalized ? GL_TRUE : GL_FALSE,
                                  layout.get_stride(), (const void *)e.offset));
    index++;
  }
...
const std::vector<hazel::core::Ref<hazel::renderer::VertexBuffer>> &
VertexArray::get_vertex_buffer() const {
  return this->vertex_buffers;
}

void VertexArray::set_index_buffer(
    const hazel::core::Ref<hazel::renderer::IndexBuffer> &buffer) {
  HAZEL_DEV_LOG_TRACE("Setting index buffer (id: {}) to vertex array (id: {})",
                      *buffer, this->renderer_id);
  gl_call(glBindVertexArray(this->renderer_id));
  buffer->bind();
...
const hazel::core::Ref<hazel::renderer::IndexBuffer> &
VertexArray::get_index_buffer() const {
