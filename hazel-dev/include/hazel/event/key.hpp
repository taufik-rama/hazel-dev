...
public:
  ...
  EVENT_TYPE_CATEGORY_METHODS(EventTypeCategorySet2(EventTypeCategory::INPUT,
...
public:
  KeyPressEvent(unsigned int key_code, int repeat_count)
      : KeyEvent(key_code), repeat_count(repeat_count) {}

  int get_repeat_count() const { return this->repeat_count; }

  std::string to_string() const override {
    std::stringstream ss;
    ss << "KeyPressEvent: " << this->key_code << " (" << this->repeat_count
       << " repeats)";
    return ss.str();
  }
